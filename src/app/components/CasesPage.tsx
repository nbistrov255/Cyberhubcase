...
export function CasesPage({ onCaseClick, isAuthenticated }: CasesPageProps) {
  ...
  // 🕵️‍♂️ DEBUG STATE: Сырые данные напрямую от API
  const [debugDirectData, setDebugDirectData] = useState<any>(null);
  const [debugError, setDebugError] = useState<string | null>(null);
  const [showDebugPanel, setShowDebugPanel] = useState(true); // Всегда показываем для диагностики
  ...
  // 🕵️‍♂️ ПРЯМОЙ ЗАПРОС К API - В ОБХОД КОНТЕКСТА
  useEffect(() => {
    const fetchDirectData = async () => {
      const token = localStorage.getItem('token');
      
      if (!token) {
        console.log('🕵️‍♂️ DIRECT API CHECK: No token found in localStorage');
        setDebugError('No authentication token found');
        return;
      }

      try {
        console.log('🕵️‍♂️ DIRECT API CHECK: Fetching from /api/profile...');
        ...
        console.log('🕵️‍♂️ DIRECT API CHECK: Response status:', response.status);

        if (!response.ok) {
          const errorText = await response.text();
          console.error('🕵️‍♂️ DIRECT API CHECK: Error response:', errorText);
          setDebugError(`API returned ${response.status}: ${errorText}`);
          return;
        }

        const data = await response.json();
        console.log('🕵️‍♂️ DIRECT API CHECK - RAW RESPONSE:', data);
        console.log('🕵️‍♂️ DIRECT API CHECK - Cases Array:', data.cases || data.profile?.cases || 'NOT FOUND');
        console.log('🕵️‍♂️ DIRECT API CHECK - Cases Length:', (data.cases || data.profile?.cases || []).length);
        
        setDebugDirectData(data);
      } catch (error) {
        console.error('🕵️‍♂️ DIRECT API CHECK: Fetch error:', error);
        setDebugError(`Fetch failed: ${error}`);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#17171c]">
      ...
      {isAuthenticated && !showDebugPanel && (
        <button
          onClick={() => setShowDebugPanel(true)}
          className="fixed bottom-6 right-6 z-50 px-4 py-3 rounded-lg font-mono text-sm font-bold transition-all hover:scale-105"
          style={{
            background: 'linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%)',
            border: '2px solid #00ff00',
            color: '#00ff00',
            boxShadow: '0 0 20px rgba(0, 255, 0, 0.3), 0 4px 12px rgba(0, 0, 0, 0.5)',
          }}
        >
          🕵️‍♂️ Open Debug Dashboard
        </button>
      )}
      ...
      <div className="mt-32 px-12 pb-20">
        <div className="max-w-[1600px] mx-auto">
          {/* 🕵️‍♂️ DEBUG DASHBOARD - Terminal Style */}
          {isAuthenticated && showDebugPanel && (
            <div 
              className="mb-12 p-6 rounded-lg border font-mono text-sm"
              style={{
                background: '#0a0a0a',
                borderColor: '#00ff00',
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-green-500/30">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-green-400 uppercase tracking-wider">
                    DEBUG DASHBOARD - API Diagnostic Tool
                  </h3>
                </div>
                <button
                  onClick={() => setShowDebugPanel(false)}
                  className="px-4 py-2 rounded bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-500/50 transition-colors"
                >
                  [X] Close
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* LEFT COLUMN: Direct API Response */}
                <div>
                  <div className="mb-3 pb-2 border-b border-green-500/30">
                    <span className="text-green-400 font-bold">&gt; RAW API RESPONSE (Direct Fetch)</span>
                  </div>
                  
                  {debugError ? (
                    <div className="p-4 bg-red-900/20 border border-red-500/50 rounded text-red-400">
                      <div className="font-bold mb-2">❌ ERROR:</div>
                      <div className="text-xs">{debugError}</div>
                    </div>
                  ) : debugDirectData ? (
                    <div className="space-y-3">
                      {/* Cases Array from Direct API */}
                      <div>
                        <div className="text-yellow-400 mb-1">📦 Cases Array:</div>
                        <div className="pl-4 text-green-300">
                          <div>Location: <span className="text-white">{debugDirectData.cases ? 'data.cases' : debugDirectData.profile?.cases ? 'data.profile.cases' : 'NOT FOUND'}</span></div>
                          <div>Length: <span className="text-white font-bold">{(debugDirectData.cases || debugDirectData.profile?.cases || []).length}</span></div>
                        </div>
                      </div>

                      {/* First Case Sample */}
                      {(debugDirectData.cases || debugDirectData.profile?.cases || []).length > 0 && (
                        <div>
                          <div className="text-yellow-400 mb-1">🔍 First Case Sample:</div>
                          <div className="pl-4 bg-black/50 p-3 rounded border border-green-500/20 overflow-auto max-h-64">
                            <pre className="text-xs text-green-300 whitespace-pre-wrap">
                              {JSON.stringify((debugDirectData.cases || debugDirectData.profile?.cases)[0], null, 2)}
                            </pre>
                          </div>
                        </div>
                      )}
                      ...
                      <details className="cursor-pointer">
                        <summary className="text-yellow-400 hover:text-yellow-300">📄 Full API Response (click to expand)</summary>
                        <div className="mt-2 pl-4 bg-black/50 p-3 rounded border border-green-500/20 overflow-auto max-h-96">
                          <pre className="text-xs text-green-300 whitespace-pre-wrap">
                            {JSON.stringify(debugDirectData, null, 2)}
                          </pre>
                        </div>
                      </details>
                    </div>
                  ) : (
                    <div className="text-gray-500 italic">⏳ Loading direct API data...</div>
                  )}
                </div>

                {/* RIGHT COLUMN: AuthContext Data */}
                <div>
                  <div className="mb-3 pb-2 border-b border-cyan-500/30">
                    <span className="text-cyan-400 font-bold">&gt; AUTH CONTEXT DATA</span>
                  </div>
                  
                  <div className="space-y-3">
                    {/* Profile Cases */}
                    <div>
                      <div className="text-yellow-400 mb-1">📦 Profile Cases:</div>
                      <div className="pl-4 text-cyan-300">
                        <div>Available: <span className="text-white font-bold">{profile?.cases ? 'YES' : 'NO'}</span></div>
                        <div>Length: <span className="text-white font-bold">{(profile?.cases || []).length}</span></div>
                      </div>
                    </div>

                    {/* First Case from Context */}
                    {(profile?.cases || []).length > 0 && (
                      <div>
                        <div className="text-yellow-400 mb-1">🔍 First Case from Context:</div>
                        <div className="pl-4 bg-black/50 p-3 rounded border border-cyan-500/20 overflow-auto max-h-64">
                          <pre className="text-xs text-cyan-300 whitespace-pre-wrap">
                            {JSON.stringify(profile.cases[0], null, 2)}
                          </pre>
                        </div>
                      </div>
                    )}
                    ...
                    {/* Processed Cases */}
                    <div>
                      <div className="text-yellow-400 mb-1">⚙️ Processed Cases:</div>
                      <div className="pl-4 text-cyan-300">
                        <div>Mapped: <span className="text-white font-bold">{cases.length}</span></div>
                        <div>Daily: <span className="text-white font-bold">{dailyCases.length}</span></div>
                        <div>Monthly: <span className="text-white font-bold">{monthlyCases.length}</span></div>
                        <div>Event: <span className="text-white font-bold">{eventCases.length}</span></div>
                        <div>Orphaned: <span className="text-white font-bold">{orphanedCases.length}</span></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6 pt-6 border-t border-purple-500/30">
                <div className="mb-3">
                  <span className="text-purple-400 font-bold">&gt; COMPARISON & DIAGNOSIS</span>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Direct API Count */}
                  <div className="p-4 rounded" style={{ background: 'rgba(0, 255, 0, 0.1)', border: '1px solid rgba(0, 255, 0, 0.3)' }}>
                    <div className="text-green-400 font-bold mb-2">📡 Direct API</div>
                    <div className="text-3xl font-bold text-white">
                      {debugDirectData ? (debugDirectData.cases || debugDirectData.profile?.cases || []).length : '⏳'}
                    </div>
                    <div className="text-xs text-green-300 mt-1">cases from server</div>
                  </div>

                  {/* Context Count */}
                  <div className="p-4 rounded" style={{ background: 'rgba(0, 255, 255, 0.1)', border: '1px solid rgba(0, 255, 255, 0.3)' }}>
                    <div className="text-cyan-400 font-bold mb-2">🔄 Auth Context</div>
                    <div className="text-3xl font-bold text-white">
                      {(profile?.cases || []).length}
                    </div>
                    <div className="text-xs text-cyan-300 mt-1">cases in context</div>
                  </div>

                  {/* Difference */}
                  <div className="p-4 rounded" style={{ 
                    background: debugDirectData && ((debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length) 
                      ? 'rgba(255, 0, 0, 0.1)' 
                      : 'rgba(0, 255, 0, 0.1)', 
                    border: debugDirectData && ((debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length)
                      ? '1px solid rgba(255, 0, 0, 0.3)' 
                      : '1px solid rgba(0, 255, 0, 0.3)' 
                  }}>
                    <div className={`font-bold mb-2 ${debugDirectData && ((debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length) ? 'text-red-400' : 'text-green-400'}`}>
                      {debugDirectData && ((debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length) ? '⚠️ MISMATCH' : '✅ MATCH'}
                    </div>
                    <div className="text-3xl font-bold text-white">
                      {debugDirectData 
                        ? Math.abs((debugDirectData.cases || debugDirectData.profile?.cases || []).length - (profile?.cases || []).length)
                        : '⏳'}
                    </div>
                    <div className="text-xs mt-1" style={{ 
                      color: debugDirectData && ((debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length) 
                        ? '#fca5a5' 
                        : '#86efac' 
                    }}>
                      difference detected
                ...
                {/* Diagnosis */}
                <div className="mt-4 p-4 rounded" style={{ background: 'rgba(147, 51, 234, 0.1)', border: '1px solid rgba(147, 51, 234, 0.3)' }}>
                  <div className="text-purple-400 font-bold mb-2">💡 DIAGNOSIS:</div>
                  <div className="text-sm text-purple-300">
                    {debugError ? (
                      <div>❌ <span className="text-red-400">API Error detected.</span> Check network tab or backend logs.</div>
                    ) : !debugDirectData ? (
                      <div>⏳ Waiting for direct API response...</div>
                    ) : (debugDirectData.cases || debugDirectData.profile?.cases || []).length === 0 ? (
                      <div>🔴 <span className="text-red-400 font-bold">BACKEND ISSUE:</span> API returned EMPTY cases array. Check database or backend logic.</div>
                    ) : (profile?.cases || []).length === 0 ? (
                      <div>🔴 <span className="text-red-400 font-bold">FRONTEND ISSUE:</span> Direct API has {(debugDirectData.cases || debugDirectData.profile?.cases || []).length} cases, but AuthContext has 0. Check AuthContext filtering logic!</div>
                    ) : (debugDirectData.cases || debugDirectData.profile?.cases || []).length !== (profile?.cases || []).length ? (
                      <div>⚠️ <span className="text-yellow-400 font-bold">PARTIAL FILTER:</span> AuthContext is filtering out {Math.abs((debugDirectData.cases || debugDirectData.profile?.cases || []).length - (profile?.cases || []).length)} cases. Check AuthContext.tsx for active filters (e.g., isActive check).</div>
                    ) : (
                      <div>✅ <span className="text-green-400 font-bold">ALL GOOD:</span> API and Context data match perfectly! {cases.length} cases are being displayed.</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 text-xs text-gray-500 text-center">
                💡 Press F12 to see detailed console logs | This panel can be closed with the [X] button above
              </div>
            </div>
          )}

...
1. **Timeout errors** - "This operation was aborted" (timeout 15 сек был слишком мал)
2. **Mock функция** - `addClientDeposit()` ничего не делала реально
3. **Неправильный API** - Использовали `setDeposit` вместо `createPayment` с типом "BONUS"
4. **Нет защиты от дублей** - Можно было нажать "ПОЛУЧИТЬ" несколько раз подряд
5. **TypeScript ошибки компиляции** - Отсутствие типизации + лишний escape символ

### ✅ Что ИСПРАВЛЕНО:

| Проблема | Решение | Файл |
|----------|---------|------|
| Timeout 15 сек | Увеличен до 30 сек | `index.ts:61` |
| Mock функция | Реальный `createPayment` с type="BONUS" | `index.ts:157-206` |
| Нет защиты | Item блокируется через `status='processing'` | `index.ts:465-478` |
| TypeScript TS18046 | Типизация `gqlRequest<{ createPayment: { id, sum } }>` | `index.ts:175` |
| TypeScript TS1002 | Убран лишний `\"` escape | `index.ts:466` |

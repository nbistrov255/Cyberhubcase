# Backend Reference Changelog

Все изменения в Backend API документируются здесь.

---

## [2024-12-30] - CRITICAL FIX: Timeout 90 сек + детальное логирование

### Changed:
- **Timeout увеличен**: 60 сек → **90 секунд** (запрос `clients(first: 10000)` требует больше времени)
- **Детальные логи** в `addClientDeposit()`:
  ```
  🔑 Service token obtained
  📡 Step 1/2: Fetching current BONUS balance...
  ✅ Step 1/2: Received 2543 clients
  📊 Current BONUS: 100€, Adding: 5€, New: 105€
  📡 Step 2/2: Setting new BONUS balance...
  ✅ BONUS updated: 105€ (added 5€)
  ```

### Why:
SmartShell API медленный при запросе большого количества клиентов (`first: 10000`).  
60 секунд timeout был недостаточен → "This operation was aborted".

---

## [2024-12-30] - ФИНАЛ: setBonus API + увеличен timeout до 60 сек

### Changed:
- **🔥 NEW API**: Функция `addClientDeposit()` переписана на использование SmartShell `setBonus` mutation
- Используется поле `bonus` (бонусный баланс) вместо `deposit` (основной баланс)
- **Timeout увеличен**: 30 сек → **60 сек** (SmartShell API медленный)

### Fixed:
- ✅ **SmartShell HTTP Error 400 ИСПРАВЛЕН**: Старый `createPayment` не работал, теперь используется правильный `setBonus`
- ✅ **Timeout errors уменьшены**: 60 секунд достаточно для большинства запросов
- ✅ **Защита от дублей работает**: Item блокируется через `status='processing'` → откат если ошибка

**Новый API:**
```graphql
mutation SetBonus($input: SetBonusInput!) {
  setBonus(input: $input) {
    uuid
    login
  }
}
variables: {
  input: {
    client_uuid: "5704047b-b551-4330-bb71-0219fd96dd89",  # UUID клиента
    value: 105  # НОВЫЙ итоговый бонусный баланс (не прибавка!)
  }
}
```

**Алгоритм:**
```typescript
1. GET текущий bonus через query { clients { data { uuid bonus } } }
2. CALCULATE: newBonus = currentBonus + amount
3. SET новый баланс через setBonus(client_uuid, newBonus)
```

### Про UUID:
`5704047b-b551-4330-bb71-0219fd96dd89` - это **ПРАВИЛЬНЫЙ UUID клиента** в SmartShell.  
`+37127177620` - это логин/телефон (не используется в API).  
SmartShell API работает **ТОЛЬКО с UUID**, не с телефонами!

---

## [2024-12-30] - ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ: TypeScript компиляция

### Fixed:
- ✅ **TypeScript Error TS18046**: Добавлена типизация для `paymentResult` → `gqlRequest<{ createPayment: { id: string; sum: number } }>`
- ✅ **TypeScript Error TS1002**: Убран лишний escape символ `\"` из строки 466 (console.log)
- ✅ **Компиляция**: Backend успешно компилируется без ошибок

---

## [2024-12-30] - ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ: createPayment с BONUS + защита от дублей

### Changed:
- **Backend**: Функция `addClientDeposit()` переписана на использование `createPayment` с типом "BONUS"
- Используется `client_id` (числовой) вместо UUID для SmartShell API
- **Timeout увеличен**: 15 сек → 30 сек (исправляет "This operation was aborted")

### Fixed:
- ✅ **РЕАЛЬНОЕ пополнение БОНУСОВ** через SmartShell GraphQL `createPayment` mutation
- ✅ **Защита от дублирования**: Item блокируется (`status='processing'`) ДО вызова API
- ✅ **Откат при ошибке**: Если `addClientDeposit()` fails → item возвращается в status='available'
- ✅ **Frontend логирование**: Добавлены console.log для отладки

**Новый алгоритм:**
```typescript
1. Проверка: item.status === 'available'
2. LOCK: UPDATE status = 'processing' (защита от double-click)
3. API: createPayment с type="BONUS", client_id, amount
4a. Успех → UPDATE status = 'received'
4b. Fail → UPDATE status = 'available' (откат)
```

**Пример createPayment:**
```graphql
mutation CreatePayment($input: CreatePaymentInput!) {
  createPayment(input: $input) {
    id sum
  }
}
variables: {
  input: {
    client_id: 12345,  # Числовой ID клиента
    sum: 5,
    cash_sum: 0,
    card_sum: 0,
    items: [{
      type: "BONUS",  # ⚡ Зачисление на бонусный счёт
      amount: 1,
      sum: 5
    }]
  }
}
```

### Security Note:
Согласно мануалу SmartShell: *"items с типом 'BONUS' говорит системе: Зачисли эти деньги не на основной депозит (DEPOSIT), а на бонусный счет"*

---

## [2024-12-30] - Исправлен баг с перезагрузкой страницы при claim денег

### Fixed:
- **Frontend Bug**: Убран `window.location.reload()` из `InventoryPage.tsx` при claim предметов типа `money`
- Теперь используется `refreshProfile()` из AuthContext для обновления баланса без перезагрузки
- Пользователь остаётся на странице инвентаря после получения денег

**Затронутые файлы:**
- `/src/app/components/InventoryPage.tsx` - строка 118

**До:**
```typescript
setTimeout(() => window.location.reload(), 1000);
```

**После:**
```typescript
await refreshProfile();
```

---

## [INITIAL] - 2024-12-30

### ✅ Создана структура Backend Reference

**Файлы:**
- `index.ts` - Полный код backend сервера
- `database.ts` - Схема SQLite базы данных
- `API_REFERENCE.md` - Документация всех endpoints
- `README.md` - Правила работы с backend reference
- `CHANGELOG.md` - История изменений

### 📊 Endpoints

#### Public (без авторизации):
- `GET /api/stats/public` - Публичная статистика
- `GET /api/drops/recent` - Последние 20 дропов для Live Feed
- `GET /api/cases/:id` - Информация о кейсе

#### Auth:
- `POST /api/auth/session` - Авторизация через SmartShell

#### User (с авторизацией):
- `GET /api/profile` - Профиль пользователя + доступные кейсы
- `POST /api/user/tradelink` - Обновить Trade Link
- `GET /api/user/history` - История открытий (50 последних)

#### Cases:
- `POST /api/cases/open` - Открыть кейс (winner определяется на сервере)

#### Inventory:
- `GET /api/inventory` - Инвентарь пользователя
- `POST /api/inventory/sell` - Продать предмет за баланс
- `POST /api/inventory/claim` - Получить предмет (деньги авто, скины → заявка)

#### Admin:
- `GET /api/admin/items` - Список всех предметов
- `POST /api/admin/items` - Создать предмет
- `DELETE /api/admin/items/:id` - Удалить предмет
- `GET /api/admin/cases` - Список всех кейсов + contents
- `POST /api/admin/cases` - Создать кейс
- `PUT /api/admin/cases/:id` - Обновить кейс
- `DELETE /api/admin/cases/:id` - Удалить кейс
- `GET /api/admin/requests` - Список заявок на вывод
- `POST /api/admin/requests/:id/approve` - Одобрить заявку
- `POST /api/admin/requests/:id/deny` - Отклонить заявку
- `POST /api/admin/requests/:id/return` - Вернуть в инвентарь

### 🗄 Database Tables

1. **sessions** - Активные сессии пользователей
2. **user_settings** - Trade Link, уровень, XP
3. **items** - Все предметы (skin/physical/money)
4. **cases** - Кейсы (daily/monthly/event)
5. **case_items** - Содержимое кейсов с весами
6. **case_claims** - Ограничения на открытие (daily/monthly)
7. **spins** - История открытий (для Live Feed)
8. **inventory** - Инвентарь пользователей
9. **requests** - Заявки на вывод скинов/физ товаров

### 🔧 Integrations

- **SmartShell API** - Авторизация, баланс, депозиты
- **Riga Timezone** - Все daily/monthly расчеты в часовом поясе `Europe/Riga`

### 🎯 Key Features

- **Weighted Random** для определения winner при открытии кейса
- **Auto-claim** для денег (прямо на SmartShell баланс)
- **Request System** для скинов/физических товаров
- **Daily/Monthly Progress** на основе SmartShell депозитов
- **Live Feed** - показывает последние 20 дропов всех игроков

---

## Формат изменений

При обновлении backend используй этот формат:

### [DATE] - Название изменения

**Added:**
- Новый endpoint `GET /api/...`
- Новая таблица `table_name`

**Changed:**
- Поле `old_name` → `new_name` в endpoint `/api/...`
- Логика расчета `feature_name`

**Fixed:**
- Баг с `description`

**Removed:**
- Удален endpoint `DELETE /api/...` (deprecated)

---

## 🔮 Planned Changes

Здесь будут записываться планируемые изменения:

- [ ] Добавить endpoint `/api/leaderboard` - топ игроков по XP
- [ ] Добавить webhook для уведомлений админа о новых заявках
- [ ] Добавить поле `notification_sent` в таблицу `requests`